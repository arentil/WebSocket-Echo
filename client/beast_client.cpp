...
int main(int argc, char** argv)
{
    ...
    std::thread cin_thread([session_ptr, &ioc](){
        std::string text;
        while (std::cin >> text) // technically its unsafe, practically this is the only std::cin used.
        {
            if (ioc.stopped() || text == "exit" || text == "stop")
            {
                boost::asio::post(ioc.get_executor(), [session_ptr](){
                    if (std::weak_ptr<session> weak_session_ptr = session_ptr; !weak_session_ptr.expired())
                        weak_session_ptr.lock()->close();
                });
                return; // exit thread func
            }
            
            boost::asio::post(ioc.get_executor(), [session_ptr, text](){
                if (std::weak_ptr<session> weak_session_ptr = session_ptr; !weak_session_ptr.expired())
